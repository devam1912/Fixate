...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  return (
    <header className="border-b border-zinc-800/80 bg-black/90 sticky top-0 z-50 backdrop-blur">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 lg:h-16 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        {/* Brand Logo */}
        <div className="flex items-center gap-3 min-w-0 w-full lg:w-auto">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-zinc-500 truncate">Failure triage, verified repairs, pull requests</p>
          </div>
        </div>

        {/* Minimalist Tab Navigation */}
        <nav className="w-full lg:w-auto flex items-center gap-1 bg-zinc-950 p-1 rounded-lg border border-zinc-800 overflow-x-auto">
          <button
            onClick={() => setActiveTab('workbench')}
            className={`flex shrink-0 items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
              activeTab === 'workbench'
